...
def ask_for_command():
    chose = ''
    appl_commands = ['p', 'l', 's', 'a', 'd', 'm', 'as', 'ss', 'bs', 'e', 'sn']
    while not chose in appl_commands:
        chose = input('\nPlease chose command listed above: ')
    return chose

# task for exception lesson
def show_all_names():
    try:
        print('All doc owners are below:')
        for doc_record in documents:
            print(doc_record["name"])
    except KeyError:
        print('Option temporary is unavailable')
...
for key in directories:
    doc_num_values.extend(list(directories[key]))
...
while True:
    ...
    if command_chosen.lower() == 'p':
        ...
        while not doc_number:
            doc_number = input('Please enter doc number: ')
            if doc_number in doc_num_values:
                doc_number = input('This № is already used. Please choose another one: ')
            else:
                continue
        while not doc_type:
